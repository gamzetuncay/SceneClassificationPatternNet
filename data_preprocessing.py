...
def split_data(folder_in, fraction_test, fraction_validation):
    ...
    for i_dir in list_classes:
        ...
        for ii in range(int(np.ceil(fraction_test * num_samples))):
            img = imgs.pop()
        ...
        for ii in range(int(np.ceil(fraction_validation * num_samples))):
            img = imgs.pop()
        ...
        print(f"{train_samples:5d} "
              f" -- Validation: {int(np.ceil(fraction_validation * num_samples)):5d} "
              f" -- Test: {int(np.ceil(fraction_test * num_samples)):5d}")
...
if __name__ == "__main__":
    ...
    datasets_dir = [os.path.join('PatternNet', 'images')]
